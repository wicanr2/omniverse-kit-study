...
> **狀態:骨架階段。** 目前 1 篇,工具鏈與分區已就緒。GitHub Pages 尚未開通,`docs/` 底下的 HTML 已建好,開通後即可直接用。
...
| 分區 | 篇數 | 收什麼 |
|---|---|---|
| [`docs/common/`](docs/common/) | 1 | 不綁 Kit 版本的機制與方法論 |
...
1 篇裡真正綁死 Kit 版本的有 0 篇,所以兩個版本區都還是空的。
...
| **extension** | `extension.toml`;相依解析與 registry;enable/disable 生命週期;「啟用成功」不是它活著;離線 registry |
...
| **執行模式** | GUI / headless / streaming;`--exec`;非同步算圖 |
